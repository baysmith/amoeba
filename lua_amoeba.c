...
{
    return aml_argferror(L, idx, "%s expected, got %s", tname,
                         luaL_typename(L, idx));
}
...
static am_Variable *aml_checkvar(lua_State *L, aml_Solver *S, int idx)
{
    ...
    if (lvar != NULL) {
        if (lvar->var == NULL)
            luaL_argerror(L, idx, "invalid variable");
        return lvar->var;
    }
...
static aml_Item aml_checkitem(lua_State *L, aml_Solver *S, int idx)
{
    ...
    case LUA_TUSERDATA:
        ...
        if (lcons) {
            if (lcons->cons == NULL)
                luaL_argerror(L, idx, "invalid constraint");
...
static aml_Solver *aml_checkitems(lua_State *L, int start, aml_Item *items)
{
    ...
    if ((lcons = (aml_Cons *)luaL_testudata(L, start + 1, AML_CONS_TYPE)) !=
        NULL) {
    ...
    if ((lvar = (aml_Var *)luaL_testudata(L, start, AML_VAR_TYPE)) != NULL) {
        if (lvar->var == NULL)
            luaL_argerror(L, start, "invalid variable");
    ...
    if ((lvar = (aml_Var *)luaL_testudata(L, start + 1, AML_VAR_TYPE)) !=
        NULL) {
        if (lvar->var == NULL)
            luaL_argerror(L, start + 1, "invalid variable");
...
static int aml_performitem(am_Constraint *cons, aml_Item *item,
                           am_Float multiplier)
{
    switch (item->type) {
    case AML_CONSTANT:
        return am_addconstant(cons, item->value * multiplier);
        break;
    case AML_VAR:
        return am_addterm(cons, item->var, multiplier);
        break;
    case AML_CONS:
        return am_mergeconstraint(cons, item->cons, multiplier);
        break;
    }
    return AM_FAILED;
}
...
static int Lexpr_mul(lua_State *L)
{
    ...
    if (items[0].type == AML_CONSTANT) {
        ...
        luaL_error(L, "attempt to multiply two expression");
    return 1;
}
...
static int Lexpr_div(lua_State *L)
{
    ...
    if (items[1].type == AML_CONSTANT) {
        ...
        luaL_error(L, "attempt to divide two expression");
    return 1;
}
...
static int Lexpr_le(lua_State *L)
{
    return Lexpr_cmp(L, AM_LESSEQUAL);
}
static int Lexpr_eq(lua_State *L)
{
    return Lexpr_cmp(L, AM_EQUAL);
}
static int Lexpr_ge(lua_State *L)
{
    return Lexpr_cmp(L, AM_GREATEQUAL);
}
...
static int Lvar_value(lua_State *L)
{
    aml_Var *lvar = (aml_Var *)luaL_checkudata(L, 1, AML_VAR_TYPE);
    if (lvar->var == NULL)
        luaL_argerror(L, 1, "invalid variable");
...
static int Lvar_tostring(lua_State *L)
{
    aml_Var *lvar = (aml_Var *)luaL_checkudata(L, 1, AML_VAR_TYPE);
    if (lvar->var)
        lua_pushfstring(L, AML_VAR_TYPE "(%p): %s = %f", lvar->var, lvar->name,
                        am_value(lvar->var));
    else
        lua_pushstring(L, AML_VAR_TYPE ": deleted");
    return 1;
}

static void open_variable(lua_State *L)
{
    luaL_Reg libs[] = {{"__neg", Lexpr_neg},
                       {"__add", Lexpr_add},
                       {"__sub", Lexpr_sub},
                       {"__mul", Lexpr_mul},
                       {"__div", Lexpr_div},
...
static int Lcons_new(lua_State *L)
{
    ...
    if (lua_gettop(L) >= 3)
        ...
        aml_newcons(L, S, aml_checkstrength(L, 2, AM_REQUIRED));
    return 1;
}
...
static int Lcons_reset(lua_State *L)
{
    aml_Cons *lcons = (aml_Cons *)luaL_checkudata(L, 1, AML_CONS_TYPE);
    if (lcons->cons == NULL)
        luaL_argerror(L, 1, "invalid constraint");
...
{
    luaL_Reg libs[] = {{"__call", Lcons_add},
                       {"__neg", Lexpr_neg},
                       {"__add", Lexpr_add},
                       {"__sub", Lexpr_sub},
                       {"__mul", Lexpr_mul},
...
static int Ltostring(lua_State *L)
{
    ...
    if (S->solver->infeasible_rows.id != 0) {
        am_Row *row =
            (am_Row *)am_gettable(&S->solver->rows, S->solver->infeasible_rows);
        luaL_addstring(&B, "\n  infeasible rows: ");
        aml_dumpkey(&B, 2, am_key(row));
        while (row != NULL) {
            luaL_addstring(&B, ", ");
...
static int Laddconstraint(lua_State *L)
{
    ...
    switch (ret) {
    case AM_UNSATISFIED:
        luaL_argerror(L, 2, "constraint unsatisfied");
    case AM_UNBOUND:
        luaL_argerror(L, 2, "constraint unbound");
    }
    return 0;
}
 ...
 * win32cc: flags+='-DLUA_BUILD_AS_DLL -shared -O3' libs+='-llua53'
 * output='amoeba.dll' */
